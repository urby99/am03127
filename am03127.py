...
def encode_charset(unicode_str):
    s = ''
    i = iter(str(unicode_str))
    for u in i:
        if u == '\033':
           s = str(s) + str( '<') + str(next(i)) + str(next(i)) + str('>')
           s=s.encode('cp1252')
...
def send_page_msg(line=1, page='A', lead=None, disp='A', wait=5, lag=None, col=None, font=None, msg=''):
    ...
    stt= str('<L%d><P%c><F%c><M%c><W%c><F%c><C%c><A%c>' % (
        line, page, lead, disp, chr(wait + 65), lag, col, font)) + str(msg)
    stt=stt.encode('cp1252')
    return stt
